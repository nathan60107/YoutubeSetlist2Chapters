...
- **自己挑要用哪則留言** — 留言區有不只一份曲目單時，每一份都會多出一個按鈕可以把章節換過去，而且這部影片會記住你的選擇
...
- **設定面板** — 開關上述這些選項
  - **選取策略** — 目前一律採用解析出最多首歌的那一則；可改成優先看頻道主置頂的留言，或指定偏好與排除哪些人寫的曲目單
  - **算是曲目單的門檻** — 一則留言要有幾首歌才當成曲目單，目前固定為 3 首
  - **預設歌曲長度** — 留言沒寫結束時間時，一首歌假設多長，目前固定為 4 分鐘
